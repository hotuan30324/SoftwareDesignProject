...
namespace BTL
{
    ...
    public class VeThuong : Ve
    {
        ...
        public DataTable hienThiThongTinVe()
        {
            string sqlstr = "select MaVe, LoaiVe, GiaVe, DiemDi, DiemDen from ThongTinVe as ttv INNER JOIN dbo.ChuyenBay AS cb ON ttv.MaCHuyenBay = cb.MaChuyenBay INNER JOIN dbo.DuongDi AS dd ON dd.MaDD = cb.MaDD where LoaiVe=N'Thường'";
            return ConnectSQL.GetData(sqlstr);
        }
        public DataTable demVeTheoLoaiTuChuyenBay(string maCB, string loaiVe)
        {
